...
```{r}
sub.training.2 <- subset(sub.training, select=-c(user_name, raw_timestamp_part_1, raw_timestamp_part_2, cvtd_timestamp, num_window))
sub.testing.2 <- subset(sub.testing, select=-c(user_name, raw_timestamp_part_1, raw_timestamp_part_2, cvtd_timestamp, num_window))
```
...
fit <- train(classe ~ ., data=cleaned.training, method="rf", trControl=trainControl(method = "cv", number = 10), preProcess=c("center", "scale"))
print(fit)
```
